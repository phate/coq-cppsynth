...
Fixpoint make_match_constructors
    ...
    : stmt_t :=
  match branches with
    | nil => stmt_block (from_list (stmt_expr (cg.throw_logic_error "impossible case" ) :: nil))
    | branch :: branches =>
...
Module oind_cpp_repr.
  Inductive t : Set :=
    | make :
      forall (tpl : tplformargs_t),
      forall (clsid : idexpr_t),
      forall (clsdecls : clsdecls_t),
      t.
End oind_cpp_repr.

Module ind_cpp_repr.
  Definition t := list oind_cpp_repr.t.
End ind_cpp_repr.
...
Definition oind_convert (oi : one_inductive.t)
  ...
  let match_fn_tpldecl := maybe_template (from_list match_fn_tplargs) match_fn_decl in
    ...
    (from_list
      ...
      (clsdecl_group
        ...
        (from_list (
          ...
          nil))) ::
      nil)) in
  oind_cpp_repr.make tpl cls_id clsdecls.

Definition ind_convert (i : inductive.t) : ind_cpp_repr.t :=
  match i with
...
Definition ind_gen_otherdecl (r : ind_cpp_repr.t) : decls_t :=
  let decls := decls_nil in
  (fix loop (decls : decls_t) (r : list oind_cpp_repr.t) {struct r} : decls_t :=
    match r with
      | nil => decls
      | cons oind r =>
...
Example oiY :=
  one_inductive.make "Y" (expr.product (Some "T"%string) sort_set sort_set) (
    (* constructors *)
