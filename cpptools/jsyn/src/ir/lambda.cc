...
{
	lambda::operation op(type, name);
	auto node = new lambda::node(parent, std::move(op));

	for (auto it = type.begin_operand(); it != type.end_operand(); it++)
		lambda::fctargument::create(node->subregion(), *it);
...
node::finalize(const std::vector<jive::output*> & results)
{
	...
	if (noutputs() > 0) {
		JSYN_ASSERT(noutputs() == 1);
		return output();
	}

	if (type().nresults() != results.size())
		throw jsyn::compilation_error("Incorrect number of results.");

	for (size_t n = 0; n < results.size(); n++) {
		auto & expected = type().result(n);
		auto & received = results[n]->type();
		if (results[n]->type() != type().result(n))
			throw jsyn::type_error(expected.debug_string(), received.debug_string());
	}

	for (const auto & origin : results)
		lambda::result::create(origin);

	return output::create(this, type());
}
...
{
	auto lambda = create(region, type(), name());
